...
{
    /// <summary>
    /// A general purpose <see cref="Stream"/> that can be written to (ahead)
    /// and read (behind) from. Uses a linked list of buffers internally.
    /// </summary>
    ...
    internal class WriteAheadReadBufferStream : Stream
    {
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            do
            {
                ...
                numberOfBytesReadFromCurrentBuffer = DoRead(
                    buffer,
                    count - totalNumberOfBytesRead,
                    currentBuffer,
                    offset + totalNumberOfBytesRead);
